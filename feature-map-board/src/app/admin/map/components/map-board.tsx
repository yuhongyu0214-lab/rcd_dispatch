...
import type { RankedCandidate } from "@/lib/dispatch/types";
...
type RecommendStatus = "idle" | "loading" | "api" | "fallback";
...
function loadAmap(amapKey: string) {
  if (window.AMap) {
...
function getMarkerContent(
  ...
  const markerColor = pointColors[point.kind];
  const style = [
    "display:block",
    "width:34px",
    "height:34px",
...
function getKindStats(
  ...
  if (activeKind === "DRIVER") {
    return [
      ...
      ["在线司机", visiblePoints.filter((point) => point.status !== "OFFLINE").length],
      ["最近刷新", refreshedAt]
    ] as const;
  }
...
export function MapBoard({ amapKey }: MapBoardProps) {
  ...
  const [mapNotice, setMapNotice] = useState(
    amapKey ? "正在加载高德地图" : "未配置高德 JS Key，已启用本地降级视图"
  );
  const [recommendStatus, setRecommendStatus] = useState<RecommendStatus>("idle");
  const [apiTopN, setApiTopN] = useState<RankedCandidate[]>([]);
  ...
  const topNCandidates: TopNCandidate[] = useMemo(() => {
    if (recommendStatus === "api" && apiTopN.length > 0) {
      return apiTopN.map((candidate, index) => ({
        ...
        activeStoreOrders: candidate.activeStoreOrders,
        activeDoorOrders: candidate.activeDoorOrders,
        priority: index + 1,
        outcome: (
          candidate.etaMinutes >= 120
            ? "MANUAL / ETA_EXCEEDED"
            : "DISPATCHED"
        ) as DispatchOutcome,
    ...
    return (payload?.drivers ?? [])
      ...
      .map((point, index) => {
        ...
        return {
          driverId: point.id,
          driverName: point.name,
          driverStatus: point.status,
          storeName: point.storeName,
          etaMinutes,
          loadPenalty,
          activeStoreOrders: 0,
  ...
  }, [recommendStatus, apiTopN, payload?.drivers]);
  ...
  useEffect(() => {
    ...
    const orderStatus = selectedPoint.status;
    if (
      orderStatus !== "PENDING" &&
    ...
    async function fetchRecommend() {
      ...
      try {
        const response = await fetch("/api/dispatch/recommend", {
          ...
          body: JSON.stringify({ orderId: selectedPoint!.id, topN: 3 })
        });
        ...
        if (result.success && result.data?.topN?.length) {
          setApiTopN(result.data.topN);
          setRecommendStatus("api");
        } else {
          setApiTopN([]);
  ...
  }, [selectedPoint?.id, selectedPoint?.kind, selectedPoint?.status]);
  ...
  useEffect(() => {
    ...
    async function renderMap() {
      try {
        ...
        markersRef.current = allPoints.map((point) => {
          const marker = new amap.Marker({
            position: [point.coordinate.lng, point.coordinate.lat],
            content: getMarkerContent(point, activeKind, selectedPoint, focusedPointIds),
  ...
  }, [activeKind, allPoints, amapKey, focusedPointIds, selectedPoint]);
  ...
  return (
    <div className={styles.viewport}>
      ...
      <aside className={styles.navRail} aria-label="调度模块">
        ...
        <Link className={styles.railItem} href="/admin/orders" title="订单池">
          单
        </Link>
        <Link className={styles.railItem} href="/admin/orders" title="车辆管理">
          车
        </Link>
        <Link className={styles.railItem} href="/admin/orders" title="司机管理">
          人
        </Link>
        <Link className={styles.railItem} href="/admin/orders" title="预警中心">
          警
        </Link>
      </aside>
      ...
      <main className={styles.mapScreen}>
        <header className={styles.mapHeader}>
          ...
          <div className={styles.fieldGrid}>
            ...
            <div>
              <span>路径规划</span>
              <strong>GET /api/map/route</strong>
            </div>
            <div>
              <span>GPS mock</span>
              <strong>GET /api/adapters/gps/mock</strong>
            </div>
          </div>
        </header>
        ...
        <section className={styles.bottomDock}>
              ...
              <>
                ...
                {selectedPoint.kind === "ORDER" ? (
                  <div className={styles.detailActions}>
                    ...
                    <Link className={styles.actionButton} href="/admin/orders">
                      派单
                    </Link>
          ...
          <article className={styles.recommendCard}>
            ...
            <div className={styles.recommendList}>
              {topNCandidates.map((candidate) => {
                const driverPoint = payload?.drivers.find(
                  (d) => d.id === candidate.driverId
                ) ?? null;
                ...
                <button
                  ...
                  className={`${styles.recommendItem} ${
                    selectedPoint?.id === candidate.driverId ? styles.recommendItemActive : ""
                  }`}
                  onClick={() => driverPoint && setSelectedPoint(driverPoint)}
                  disabled={!driverPoint}
                >
                  <strong>
                    #{candidate.priority} {candidate.driverName}
                  </strong>
                  {candidate.reason ? (
                    <p className={styles.reasonText}>{candidate.reason}</p>
                  ) : null}
                  ...
                  <div className={styles.candidateRow}>
                    <span>状态</span>
                    <span>{statusLabels[candidate.driverStatus] ?? candidate.driverStatus}</span>
                  </div>
                  <div className={styles.candidateRow}>
                    <span>门店</span>
                    <span>{candidate.storeName}</span>
                  </div>
                  <div className={styles.candidateRow}>
                    <span>结果</span>
