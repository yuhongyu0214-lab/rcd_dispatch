...
> 版本戳：`RCD-DOC-REGISTRY-20260816-R32`
> 建立日期：2026-07-13
> 治理更新时间：2026-08-16
...
| 项目当前状态 | 2026-08-16 | Gate 3 `PASS`；唯一串行 API 候选 `152f7c5…` 代码审计 `PASS / READY_FOR_T1`，2A/2B/2C 未启动 | [状态总览](../status/README.md) |
| Agent 公共上下文 | RCD-AGENT-CONTEXT-20260816-R31 | Layer 0；所有 Agent 必读；不超过 150 行；T1 候选、基线和两段验收已登记 | [Agent 公共上下文](../context/agent-common-context.md) |
...
5. 主控已于 2026-08-11 最终裁决 Gate 3 `PASS`。2026-08-16 唯一串行 API 候选 `152f7c5142131a030be560a863285eb6d32d0a2f` 已基于正式代码基线 `0e8ea7fc70d523de2cfa81173f1a421cad16ade7` 完成代码审计，数据补丁 `c6850df…` 位于祖先链，状态为 `PASS / READY_FOR_T1`；`60f03af…` 已废止。T1、主控合并裁决和合并后全量回归完成前，Web、司机端和观测三分支不得启动；远程仍为 `ae471484…`，且不得执行预生产 migration、部署、基础资料或证据清理。
