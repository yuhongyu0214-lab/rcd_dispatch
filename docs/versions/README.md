...
> 版本戳：`RCD-DOC-REGISTRY-20260816-R29`
> 建立日期：2026-07-13
> 治理更新时间：2026-08-16
...
| API 契约 | V2.0-r15 | 串行审计返修已冻结：完整人工计划、地址重编码、严格版本冲突及三项调度员读 DTO | [API 契约 V2](v2.0/api-contract-v2.md) |
...
| 项目当前状态 | 2026-08-16 | Gate 3 `PASS`；串行 API 候选 `319f734…` 审计 `FAIL`，第二轮进入数据模型约束返修前置 | [状态总览](../status/README.md) |
| Agent 公共上下文 | RCD-AGENT-CONTEXT-20260816-R28 | Layer 0；所有 Agent 必读；不超过 150 行；返修正式基线完整 SHA 由主控任务卡登记 | [Agent 公共上下文](../context/agent-common-context.md) |
...
5. 主控已于 2026-08-11 最终裁决 Gate 3 `PASS`。2026-08-16 串行 API 候选 `319f73401359ef4a232a2217afaaef2ef4212af2` 经代码审计判定 `FAIL`，不得合入或进入正式测试；API r15 已冻结返修语义。当前先由 `feature/v2-dispatch-event-constraint` 独立新增 outbox CHECK forward/rollback migration，合入新基线后再返修 API 候选。两轮返修、复审和全量验收完成前，Web、司机端和观测三分支不得启动；远程仍为 `ae471484…`，稍后处理，且不得执行预生产 migration、部署、基础资料或证据清理。
