...
> 版本戳：`RCD-DOC-REGISTRY-20260827-R50`
> 建立日期：2026-07-13
> 治理更新时间：2026-08-27
...
| 项目当前状态 | 2026-08-27 | Gate 2 兼容返修已合入本地 develop；3A/3B 旧基线失效，等待从新统一基线重验 | [状态总览](../status/README.md) |
| Agent 公共上下文 | RCD-AGENT-CONTEXT-20260827-R49 | Layer 0；所有 Agent 必读；不超过 150 行；登记 Gate 2 接收与 3A/3B 重置 | [Agent 公共上下文](../context/agent-common-context.md) |
...
状态统一为 `GATE2_REMEDIATION_MERGED_LOCAL / NEW_BASELINE_FROZEN / 3A_3B_REVALIDATION_PENDING`。Gate 2 候选 `8fdfad7…` 已合入本地 `develop @ a852c4f…` 并通过 `808/7`、lint、31/31 build 与 diff check；旧 `50525878…` 上的 3A/3B 首轮证据不再是 Gate 4 入口证据。新统一基线定义为本治理文档所在提交，其完整 SHA 由主控提交后下发（同一提交内不自引用），不得猜测；`origin/develop @ ae471484…` 未推送。

| 验证线 | 角色与入口 | 初始边界 | 隔离候选与结果摘要 |
|---|---|---|---|
| 3A | 数据库验证 Agent；`feature/v2-migration-validation`；`.worktrees/v2-migration-validation` | 旧首轮结果已触发并完成 Gate 2 返修；新基线修改白名单重新归零 | PostgreSQL 18.3 `127.0.0.1:55437`；从新基线复验 10 个 migration、迁移映射、双读/兼容窗口、数据核对、rollback 与零漂移 |
| 3B | 测试 Agent；`feature/v2-e2e-validation`；`.worktrees/v2-e2e-validation` | 首轮 FAIL 保留；T0 稳定化返修必须先在独立分支闭合并经主控合入 | PostgreSQL 18.3 `127.0.0.1:55438`、app `3048`、Mock `3049`；新基线上重验 PRD §13 完整 14 项、故障、浏览器及 test/lint/tsc/build/diff check |

两条现有干净 worktree 必须快进到本治理文档所在提交，不继承旧白名单或授权，且 Agent 不更新治理文档。主控提交后下发完整 `BASELINE_SHA`；3A 可从新基线重新开始 T0，3B 必须先等待 T0 稳定化返修进入同一统一基线。两线仍无外部系统授权。
...
5. 主控已于 2026-08-11 最终裁决 Gate 3 `PASS`；第二轮与 P2 随后退出。2026-08-27，3A 首轮发现的 Gate 2 V1 写兼容窗口缺口由 `8fdfad7…` 闭合并以 `--no-ff` 合入本地 `develop @ a852c4f…`，合入后 `808 passed / 7 expected skipped`、lint、31/31 build 与 diff check 通过。3A/3B 旧 `50525878…` 验收起点失效；本治理提交形成新统一 SHA 后，两线必须重新对齐并验收，全部通过前不得进入 Gate 4。远程仍为 `ae471484…`。
