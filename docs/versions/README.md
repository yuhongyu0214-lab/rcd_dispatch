...
> 版本戳：`RCD-DOC-REGISTRY-20260823-R39`
> 建立日期：2026-07-13
> 治理更新时间：2026-08-23
...
| API 契约 | V2.0-r17（develop 现行） | 2B 候选 `c1279b5…` 内含 r18 的 `DriverTaskV2.servicePlan` 收口；待审计、测试和合并后才进入 develop 现行入口 | [API 契约 V2](v2.0/api-contract-v2.md) |
...
| 项目当前状态 | 2026-08-23 | 2C 已合入本地 `develop @ 04aba179…` 并通过合入后回归；2B 为下一合并对象，2A 在其后 | [状态总览](../status/README.md) |
| Agent 公共上下文 | RCD-AGENT-CONTEXT-20260823-R38 | Layer 0；所有 Agent 必读；不超过 150 行；2C 合入事实与 2B 新基线规则已登记 | [Agent 公共上下文](../context/agent-common-context.md) |
...
5. 主控已于 2026-08-11 最终裁决 Gate 3 `PASS`。2026-08-16 串行 API 候选完成 T1 并合入，第二轮初始统一代码基线为 `develop @ f44afac43282463cd9d7cce9e13668feedf8a133`。2026-08-23，2C 原候选 `99d6909…` 以相同 patch 线性重放为 `04aba179…`，随后 `--ff-only` 合入本地 develop；专项 `35/35`，合入前后全量均为 `648 passed / 7 expected skipped`，lint、`29/29` build 和 diff check 通过。2B 为下一合并对象，必须 rebase 到本轮治理文档提交后的最终 develop HEAD；2A `6562544…` 已 `EXIT_PASS`，在 2B 后合入。远程仍为 `ae471484…`。
