...
> 决策版本：`RCD-APP-DECISIONS-V2.0-R9-20260915`
> 状态：APP-008 已进入通过独立审计的联合账号/入口候选；预生产仍运行 APP-007，受邀注册尚未部署
> 代码事实：本地联合候选 `40b4a0fca3c9f9b4cfd392341f89663fdbbcb418`；预生产运行 `codex/preprod-login-mobile-remediation @ b2887d3718f34bf3cc068d07b505d33065e77c7c`
...
- 后继联合实现：`40b4a0fca3c9f9b4cfd392341f89663fdbbcb418` 已在 APP-008 上串行加入受邀注册、双端退出和可信原始 URI 精确入口治理，通过浏览器与独立审计。预生产仍未切换，公开注册继续关闭；发布必须使用服务端邀请密钥和受控最小 User INSERT/指定列 UPDATE，不得切换开发模式或授予 app 通用 User 写权限。

## 版本记录

| 版本 | 日期       | 内容                                                                                                                                |
| ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| V2.0-r9 | 2026-09-15 | APP-008 后继联合候选加入受邀注册、双端退出与原始 URI 入口治理并通过本地审计；框架/依赖不变，预生产仍为 APP-007，未授权部署 |
