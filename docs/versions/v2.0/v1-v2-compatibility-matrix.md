...
V1 `DriverStatus`（6 值）→ V2 班次 + 位置新鲜度 + 执行推导：

| V1 | V2 onShift | 说明 |
|---|---|---|
| `OFFLINE` | `false` | 未当班 |
| `S1`（门店空闲） | `true` | 空闲；V2 不再区分门店/返程，位置以实时定位为准 |
| `S2`（返程空闲） | `true` | 同上 |
| `S3`（门店忙碌） | `true` | 忙碌状态由 V2 从 A 槽位工单执行状态推导，不再单独存储 |
| `S4`（订单忙碌） | `true` | 同上 |
| `UNAVAILABLE` | `false` | 不参与调度；迁移后映射为未当班 + 备注保留原值 |
...
| `RECYCLED` | 历史链记录 | 同上 |
| `COMPLETED` | 有效 Assignment（`COMPLETED`） | `completedAt` 取 V1 完成时间 |
| `CANCELLED` | 历史链记录 | 同上 |

新增字段的默认回填：`sequenceNo` 按现存有效 Assignment 的创建时间排序生成；`planVersion` 初始为 1；计划时间与 ETA 字段留空，由首次重算填充。
...
| `Order.channel` | `sourceSystem` | 值映射（如 `BROWSER_PLUGIN → PLUGIN`），映射表在 Gate 1 定稿 |
| （无） | `externalOrderId` | 存量订单回填为 `orderNo`（标记 `backfilled=true`） |
| （无） | `sourceVersion` | 存量回填 `"v1-migration"` |
| （无） | `receivedAt` | 存量回填 `createdAt` |
| `Order.status` | `executionStatus` + 维度 | 按 §1 映射 |
| `Driver.status` | `DriverShift` + 派生 | 按 §2 映射 |
...
| `COMPLETE` | 保留 |
| （新增） | `AUTO_DISPATCH / DEPART / ARRIVE / MODULE_CHANGE / ORDER_MODIFY / ALERT_RESOLVE / SHIFT_START / SHIFT_END / UNLOCK` |
