...
- **保留值例外（冻结）**：上述两种格式适用于 `HALUO / PLUGIN / API` 在线投递。`V1_IMPORT` 迁移事件固定使用保留值 `"v1-migration"`：不经过在线 ingest（在线投递 `V1_IMPORT` 被拒，见 API 契约 §2.3），不参与跨版本新旧比较；由于 `sourceSystem` 独立，不会与在线来源的版本发生排序。
