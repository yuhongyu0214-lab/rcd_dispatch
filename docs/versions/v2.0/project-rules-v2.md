# 人车单项目代码与设计规则 V2

> 规则版本：`RCD-RULES-V2.0-R3-20260730`
> 状态：Gate 0 主体与 R4-Demo 补充规则已由 Gate 3-R R7 统一冻结
> 作用：统一代码一致性、开发边界、设计变量和 V1/V2 冲突处理

## 1. 规则来源与归档结论

| 来源 | 保留内容 | V2 覆盖内容 |
|---|---|---|
| `AGENTS.md` / `CLAUDE.md` | `AGENTS.md` 中的 pnpm、技术栈、分支、worktree、API、日志、命名和测试；`CLAUDE.md` 仅作工具兼容参考 | `CLAUDE.md` 与当前权威入口冲突的内容、V1 阶段名与 V1 业务验收 |
| V1 守门规范 | 数据安全、边界合规、文档优先、事务和审查等级 | V1 状态机、S1-S4 业务解释、Top N、120 分钟阈值 |
| `PRODUCT.md`（历史参考） | 克制、专业、调度控制台的产品性格 | 旧业务目的描述 |
| `docs/demo-v12-ui-ux-spec.md`（历史参考） | 视觉 token、三栏布局、内部滚动、状态不只靠颜色 | Top N dock、车辆调度和旧页面行为 |
| `docs/demo-v12-api-contract.md`（历史参考） | 统一响应、traceId、分页和错误状态 | V1 DTO、接单、Top N 和旧生命周期 |

## 2. 文档优先级

文档权威按领域拆分，唯一口径见 [文档版本总入口](../README.md)「文档权威顺序」一节；本文件是其中“代码一致性与设计系统”领域的权威。本文件不整份覆盖其他文档，其他文档也不整份覆盖本文件；跨领域冲突无法判断归属时暂停实现并升级裁决。

旧文档是否仍保留在原路径，不影响其权威状态；凡未列入版本总入口“当前有效版本”的旧方案、Prompt、报告和 runbook，均不得指导新的 V2 实现。

## 3. 代码一致性规则

### 3.1 继续锁定

- Next.js 14 App Router + TypeScript。
- Tailwind CSS + shadcn/ui，不引入第二套 UI 框架。
- PostgreSQL + Prisma。
- Redis/Tair 只保存实时或短期数据，数据库保存业务事实。
- 高德服务端 Key 只在服务端；JS Key 只用于地图渲染。
- Pino 结构化日志，禁止 `console.log`。
- pnpm `10.11.0`，禁止 npm/yarn 和新增 `package-lock.json`。
- API 使用 `ok()` / `fail()`，统一返回 `{ success, data, error, traceId }`。
- 枚举 UPPER_SNAKE_CASE，函数/变量 camelCase，文件 kebab-case，组件 PascalCase。

### 3.2 V2 新增硬规则

- 外部订单字段只能存在于 Adapter 和来源事件层；调度代码只接收 CanonicalOrder 或内部 DTO。
- 外部原始状态不得直接写入内部执行状态。
- 执行状态、可行性、锁定和 A/B/C 槽位必须分开建模。
- 车辆字段不得进入 V2 候选过滤、评分和迟到判断。
- 自动重排必须使用司机计划聚合的 `planVersion` 或等价乐观锁（归属定义见数据架构 §6）。
- 已到达工单的改派必须在服务端拒绝，不能只禁用前端按钮。
- ETA 不可用时返回明确原因，不得使用假数据。
- 人工订单修改保存原值、新值、操作者、时间、原因和 traceId。
- V2 代码不得复用含 V1 业务语义的类型名而不写兼容说明。

## 4. 开发与分支边界

- V1 文件与主线暂不删除，现有 bug 修复仍按原 worktree 范围执行。
- V2 新功能使用 `feature/v2-*` 分支或独立 worktree，不把 V2 schema 与 V1 页面修改混进同一提交。
- 合并路径仍为 `feature/* → develop → main`。
- V2 顺序：文档 → schema → 内部 DTO/API → Adapter → 调度引擎 → 页面 → 观测与稳定性。
- 当前文档阶段只修改 Markdown；本版本不授权修改 `.ts/.tsx/.prisma`。
- Schema 变更先生成迁移 SQL 和 rollback SQL，再等待审查。
- 每阶段退出前通过测试、构建和业务验收，不以“页面能打开”替代闭环验收。

## 5. 设计系统 V2

### 5.1 产品气质

克制、专业、信息密集的调度控制台。禁止回退为普通蓝白 SaaS、营销页、大卡片堆叠、过圆组件和无意义动画。

### 5.2 继承的设计变量

| Token | 值 | 用途 |
|---|---:|---|
| `--bg` | `oklch(0.945 0.006 235)` | 页面背景 |
| `--panel` | `oklch(0.905 0.012 235)` | 工作面板 |
| `--surface` | `oklch(0.985 0.003 235)` | 表格、卡片和输入区 |
| `--map` | `oklch(0.875 0.018 230)` | 地图底色 |
| `--nav` | `oklch(0.235 0.032 240)` | 深色导航 |
| `--accent` | `oklch(0.635 0.115 35)` | 主操作和选中态 |
| `--success` | `oklch(0.590 0.115 160)` | 可行、在线、时间充足 |
| `--warning` | `oklch(0.690 0.125 76)` | 风险、等待处理 |
| `--danger` | `oklch(0.610 0.160 27)` | 不可行、超时和危险操作 |
| `--info` | `oklch(0.620 0.090 220)` | 中性业务信息 |

| 尺寸 | 值 |
|---|---:|
| 导航 rail | 72px |
| 工作面板 | 420px |
| 控件高度 | 38px |
| 最小点击目标 | 44px |
| 小/中/大圆角 | 8px / 12px / 16px |

数值来源：`docs/demo-v12-ui-ux-spec.md` 和 `feature-admin-workflow/.extract-design-system/demo_v12-design-parameters.md`。

### 5.3 V2 布局规则

- Web 保留“深色导航 + 左侧工作面板 + 右侧主工作区”。
- 全局地图是核心工作区，不得被订单表格完全替代。
- 调度主视图同屏展示地图和选中司机 A/B/C 时间轴。
- 时间轴区分空驶 ETA、固定模块、工单驾驶、空闲和迟到风险。
- 司机位置显示更新时间；过期时同时使用灰色、图标和文字。
- 不可行订单在地图、列表和时间轴至少两处可见，不能只藏在日志页。
- 颜色只作辅助，红/绿模块同时显示“时间不足/时间充足”。
- 页面级不滚动，密集内容使用模块内部滚动；宽表格使用横向滚动。
- 选中对象联动地图、列表和时间轴，不通过跳页破坏上下文。
- 候选解释面板以“司机 + 建议槽位”为一行，默认展示前三名并可展开更多；必须显示真实 ETA 分钟数、优先级和不可替代的文字标签。
- 地图路线区分空驶段与工单段，并与候选排序使用同一次服务端 ETA 结果。
- 派单、改派、撤回、解除锁定和订单修改使用 Toast 提供即时成功/失败反馈；Toast 不承载持续预警、操作审计或需要用户后续处理的事项。
- 定位 H5 首版保持单任务界面，只显示定位授权、开始/停止、最近上报时间、精度和状态；不得提前混入 V1 接单流程。

### 5.4 V1 设计行为的替换

| V1 行为 | V2 行为 |
|---|---|
| 地图底部 Top N 推荐卡 | 司机 A/B/C 时间轴与重排结果 |
| 点击候选司机选中 | 系统自动排程，调度员可手动改派 |
| 车辆作为地图调度对象 | 车辆仍可查看，但不参与算法 |
| 司机“接单”操作 | 出发、到达、完成三步执行 |
| 普通 ETA 超限提示 | 持续的不可行预警和修改入口 |

## 6. API 与状态规则

- V2 API 契约以 [api-contract-v2.md](api-contract-v2.md) 为 HTTP 契约领域的唯一权威，不直接改写 `demo-v12-api-contract.md`。
- 内部路径保持稳定，通过 DTO 兼容，不把外部 API 结构透传给页面。
- 并发冲突返回 409 和冲突司机当前 `planVersion`（改派返回原/目标司机两个版本）。
- 非法状态流转返回 400，并包含 `currentStatus` 和 `targetStatus`（流转矩阵见 PRD V2 §10.1）。
- 外部依赖故障返回可理解错误，详细原因写结构化日志。
- 所有时间使用带时区 ISO 8601；数据库统一存 UTC，前端按 Asia/Shanghai 展示。

## 7. 审查清单

### P0 阻断

- 是否跨越 worktree 或阶段范围。
- 是否绕过 CanonicalOrder 直接使用外部字段。
- 是否允许算法覆盖锁定或执行中工单。
- 是否允许到达后改派。
- 是否存在重复排程、重复派单或非幂等入库。
- 是否使用假 ETA 或把过期位置当实时位置。

### P1 严重

- API 是否统一响应并带 traceId。
- 自动/人工变更是否有结构化日志与前后值。
- 状态、可行性、锁定、槽位是否混为一个字段。
- 车辆是否误入 V2 匹配逻辑。
- UI 是否隐藏不可行预警或只靠颜色表达。

### P2 建议

- 组件是否遵守既有 token 和信息密度。
- 是否增加未被 PRD 要求的抽象或配置表。
- 是否有可删除的 V1 兼容分支；删除前必须有迁移验收和用户批准。

## 8. 版本记录

| 版本 | 日期 | 内容 |
|---|---|---|
| V2.0 | 2026-07-13 | 汇总代码一致性、设计变量、设计规则及 V1/V2 冲突裁决 |
| V2.0-r1 | 2026-07-17 | Gate 0 审查修订：§6 指向 api-contract-v2.md 唯一权威版本，补充流转矩阵引用 |
| V2.0-r2 | 2026-07-17 | Gate 0 二轮返修：§2 改为按领域拆分的权威顺序（唯一口径在文档版本总入口）；§3.2/§6 对齐 `planVersion` 司机计划聚合归属与改派双版本 |
| V2.0-r3 | 2026-07-30 | R4-Demo 视觉与交互补充：候选优先级、同源路线、操作 Toast 与极简定位 H5 边界 |
