...
> 规则版本：`RCD-RULES-V2.0-R21-20260913`
> 状态：当前有效
...
> 本地联合候选：`feature/v2-account-entry-integration @ bf9aeefcd3be02fdd08be0f42a4d165ef2d39765`；一号双端与 V2 默认入口已完成工程及本地 HTTP 验证，真实浏览器矩阵受控制连接故障阻塞，未部署
...
- 入口治理按 PRD §9.1.1 与兼容矩阵 §7.1 实施：普通旧 Web 地址只映射到 V2；四类精确单值 `mode` 才可进入隐藏工具；V2 正常导航不得提供 V1 回程。已接受的旧组件内部切换 P2 不得被解释为可扩大入口或删除 V1 API/Adapter 的授权。
