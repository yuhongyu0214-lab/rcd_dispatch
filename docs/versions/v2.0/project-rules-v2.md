...
> 规则版本：`RCD-RULES-V2.0-R22-20260915`
> 状态：当前有效
...
> 本地联合候选：`feature/v2-account-entry-integration @ 40b4a0fca3c9f9b4cfd392341f89663fdbbcb418`；一号双端、V2 默认入口、受邀注册与双端退出已完成自动化、隔离 HTTP、Chrome/Edge 与独立审计，未部署
...
- 生产环境禁止公开管理员注册，不得预填或展示演示账号密码；受邀注册只有在至少 32 字符的服务端签名密钥存在时开放，邀请码必须绑定手机号和有效期。开发环境公开注册也必须由显式开关启用。
...
- 一号双端按 PRD §7.4～7.6 与 API r19 实施：工作台能力与历史 role 分离，H5 本人资源守卫继续有效；注册/补档案写入集中在 Serializable 应用事务，不从前端接收可冒用的 role、userId 或 driverId，补档案手机号只取当前登录身份。受邀注册必须保持密钥闸门、HMAC 手机号/有效期校验、同源检查与最小数据库权限，未配置时失败关闭。
- 入口治理按 PRD §9.1.1 与兼容矩阵 §7.1 实施：普通旧 Web 地址只映射到 V2；隐藏工具必须同时匹配可信原始 URI 与框架规范化 URI 的四条精确值。公网入口必须覆盖客户端同名原始 URI 头，app 不直接发布公网端口；编码、额外/重复参数、尾斜杠和缺少可信头一律失败关闭。已接受的旧组件内部切换 P2 不得被解释为可扩大入口或删除 V1 API/Adapter 的授权。
- 安全登录目标必须由集中路由策略解析；拒绝站外、协议相对、反斜杠、控制字符、编码目录穿越、登录自循环与多值参数。历史纯司机默认 H5，admin/dispatcher 默认 V2；角色默认目标不改变 PRD §7.4 的双端授权范围。
- 退出登录复用统一会话清理接口，不得顺带结束司机班次或改写调度事实；网络和非 2xx 故障应可反馈，当前缺少显式反馈作为已知 P2 跟踪。
...
| V2.0-r21 | 2026-09-13 | 登记联合候选 `bf9aeef…`，增加 V2 默认入口、精确隐藏模式、安全 next 与已接受 P2 边界；API 契约、Schema、migration、依赖和设计变量不变 |
| V2.0-r22 | 2026-09-15 | 登记联合候选 `40b4a0f…`：受邀注册、双端退出和可信原始 URI 双重精确入口通过浏览器与独立审计；保留两项 P2，Schema、migration、调度核心和设计变量不变 |
