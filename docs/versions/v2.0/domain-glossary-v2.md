...
> 词汇版本：`RCD-GLOSSARY-V2.0-R13-20260810`
> 状态：Gate 3 应用候选枚举复核通过
...
| 基线校验 | 每 10 分钟对全部计划做一次校验性重算；周期 worker 按 UTC 十分钟稳定桶生成唯一 `BASELINE_RECALCULATION` outbox 事件 |
| `DispatchEventOutbox` | 内部调度事件的事务型持久队列；与产生事件的业务事实同事务写入，以稳定 `eventId` 去重，至少一次消费，失败保留并退避重试。它不属于订单来源，不得复用 `OrderSourceEvent` 或扩展 `sourceSystem` |

### 6.1 Gate 3 候选枚举一致性

应用候选的依赖安全返修没有修改 Prisma Schema、V2 DTO 或任何业务枚举。执行状态、可行性、锁定、槽位、来源、服务模块、预警和调度事件类型继续以本文件及 API 契约为唯一口径；Next.js / React 版本不参与领域枚举定义。
...
| V2.0-r5 | 2026-07-26 | Gate 3-2 审查返修：新增 `DispatchEventOutbox` 术语，明确内部调度事件与外部订单来源的边界 |
| V2.0-r6 | 2026-07-26 | Gate 3-2 二次返修：明确十分钟基线以 UTC 稳定时间桶生成唯一内部事件 |
| V2.0-r7 | 2026-08-01 | 对齐应用候选 `7f60fd0d55783d1f057c814e46a3dab7f73e2416`：确认依赖安全返修未修改 Prisma/V2 枚举，框架版本与领域枚举保持正交 |
| V2.0-r8 | 2026-08-01 | 对齐最终候选 `3dea9260865f7e6ed42938d83e370e3823d31a2d`：rollback 顺序修复未修改领域术语或枚举 |
| V2.0-r9 | 2026-08-02 | 对齐部署可用性返修后的唯一候选 `169f2ad8b27f9f0be2d4630144315694656b6a67`：容器、健康检查与 ACR 发布未修改领域术语或枚举 |
| V2.0-r10 | 2026-08-02 | 对齐部署入口加固候选 `7378303f513d92e781a7930cfff7e14269ec3126`：Compose/Nginx 加固未修改领域术语或枚举 |
| V2.0-r11 | 2026-08-03 | 对齐 Compose 命令修正候选 `492c86ea51b40da9426b8ad5b6aef861aa429ab5`：部署 README 与测试修正未修改领域术语或枚举 |
| V2.0-r12 | 2026-08-08 | 对齐可观测性候选 `4eb3b4857caae9730eb70dd9f2fb152bd8972ca0`：日志与部署追溯返修未修改领域术语或枚举 |
| V2.0-r13 | 2026-08-10 | 对齐当前候选 `958afca537b412fb972b6e180561a9b37022834d`：真实 10 单、并发、手工改排、故障与回退验收未修改领域术语或枚举 |
