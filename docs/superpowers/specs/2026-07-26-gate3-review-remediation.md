...
> **历史状态（2026-07-30）：SUPERSEDED。** 本文保留 Gate 3 返修与 Railway 私有 Demo 的真实证据，但其中“Railway 升格为正式生产平台”的裁决已被 Gate 3-R 替代。阿里云是目标正式生产主线；阿里云尚未完成的部署能力不得从本文推定为已验收。下文保留当时原始记录，不再作为当前部署指令。
>
> 审查基线：`feature/v2-dispatch-integration @ cd4bca5`
>
> 返修分支：`feature/v2-gate3-review-remediation`
>
> 当前结论：代码级 P0/P1 与 G3-3 编排、锁、事务提交及本地并发验收已闭环；PostgreSQL 18 隔离库迁移/rollback、真实数据库 outbox 并发、真实 Redis 锁竞争、高德真实成功/无路径及可控超时、空路径、`10003` 反例已有证据。阿里云集成主库已在全量备份完成后执行唯一待迁移项并通过数据库核对。Railway 私有 Demo 已完成 app、隔离 PostgreSQL/Redis 和每分钟 worker 部署，形成连续 60 秒调用证据。2026-07-29 用户裁决 Railway 在 Gate 3 内升格为正式 app/worker 运行平台，但当前部署仍来自未提交返修工作区、使用 Limited Trial，且未完成 Railway→阿里云 RDS/Redis 的固定出站最小白名单和生产登录面安全闭环，因此仍不能冒充正式发布。Gate 3 不提前宣布 `PASS`，第二轮并行继续冻结。
...
1. 用可追溯且获准的 Git 提交替换 app/worker 当前 “via CLI / local remediation worktree” 部署；记录提交 SHA、两个部署 ID 与可回滚的上一部署 ID。
2. 将 Railway 工作区从 Limited Trial 切换为不会因试用到期停服的套餐。若正式 app 直连要求 IP 白名单的阿里云 RDS/Redis，必须采用固定出站 IP 或等价最小边界；不得把阿里云白名单开放为全网。
3. app 正式变量补齐 `AMAP_SERVER_KEY`、地图 JS Key/Security Code、`CORS_ORIGINS`，并确认 `DATABASE_URL`/`REDIS_URL` 指向获准的阿里云正式目标；只核对变量名和连接结果，不把密钥值写入日志、截图或本文。
4. 生产环境关闭公开管理员注册，登录页不预填或展示默认种子账号密码；正式管理员凭据必须独立轮换。
5. 保持 worker `Public Networking: disabled`、`Replicas: 1`，与 app 共用独立 `INTERNAL_CRON_SECRET`，并重新形成连续周期调用、`failed=0`、健康检查、数据库迁移状态和日志脱敏证据。
