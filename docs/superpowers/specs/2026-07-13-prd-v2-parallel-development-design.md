...
- 选中订单时展示通过硬条件与可行性检查的“司机 + 槽位”候选，按真实衔接 ETA 从低到高排列，默认前三名可展开更多。
- 地图展示司机/前序结束位置到取车点的空驶路线，以及取车点到送达点的工单路线；路线与候选 ETA 使用同一次服务端高德结果。
- 派单、改派、撤回、解除锁定和订单修改具有清晰 Toast；持续预警和操作日志保留独立入口。
- 保留深色导航、灰蓝背景、暖锈强调色和克制的信息密度。
- 导航 rail 72px、工作面板 420px、控件 38px、点击目标至少 44px。
- 页面级不滚动，密集模块内部滚动。
- 状态同时使用颜色、图标和中文标签。

### 7.2 并行线 2B：司机执行接口与首版定位 H5
...
- 极简司机定位 H5：身份验证、开始/停止定位、前台持续采集、定位状态与恢复前台后立即上报
...
- 执行中司机不能下班。
- 正式 V2 Demo 使用真实手机前台定位；锁屏或进入后台后允许暂停，不作为验收失败。
- 首版 H5 不展示订单、工单、其他司机、接单、导航或执行按钮；完整司机业务端不纳入正式 V2 Demo。
...
- 真实手机定位 H5、候选 ETA 排序、同源路线、A/B/C 时间占用和 Toast 均有明确端到端验证记录。
...
- 六步以上的正式演示脚本完整走通。
- 正式演示至少包含：手机前台真实定位、订单/司机地图关系、真实高德 ETA 与路线、司机+槽位优先级、真实派单、A/B/C 时间占用和清晰操作反馈。
- 数据迁移、回滚、并发和故障场景验证通过。
- `develop` 可运行、可演示、可回滚。
...
### 13.1 Gate 3-R 暂停与 R4-Demo 口径修订

- 2026-07-29 起，Gate 3 与第二轮并行因国内生产基建重新冻结而暂停；Railway 降级为历史 Demo 证据，阿里云成为目标生产主线，是否已部署或验收以 Gate 3-R 档案为准。
- 2026-07-30，用户批准正式 V2 Demo 增加可提前部署的极简司机定位 H5：仅在页面前台采集真实手机位置，锁屏或后台不要求持续定位。
- 同次修订冻结调度员 Demo 的候选解释、同源高德路线、A/B/C 时间占用和 Toast 要求；完整司机业务 H5 仍不纳入首版正式 Demo。
- Gate 3-R 的 R0～R7 已于 2026-07-30 完成，四份基建档案、R4-Demo 修订和权威桥接已冻结；这只表示图纸定稿，不代表阿里云已部署或 Gate 3 已重新通过。
- 以上修订先完成文档一致性与 Gate 3-R 冻结，第二轮分支不得因此提前创建。
